...
from .lorarar import MergedLoRALinearLayer, MergedLoRALinearLayerInference
...
def initialize_merged_lora_layer(state_dict, state_dict_2, part, **model_kwargs):
    merged_lora_layer = MergedLoRALinearLayer(**model_kwargs, part=part)
    merged_lora_layer.load_state_dict(
        {
    ...
    return merged_lora_layer


def unet_merged_lora_state_dict(
    ...
    for name, module in unet.named_modules():
        if hasattr(module, "set_lora_layer"):
            ...
            if lora_layer is not None:
                assert hasattr(lora_layer, "get_merged_lora_weight"), lora_layer
                weight = lora_layer.get_merged_lora_weight()
...
def merged_lora_set_forward_type(unet: UNet2DConditionModel, type: str = "merge"):
...
def merged_lora_compute_mergers_similarity(unet):
    similarities = []
    for name, module in unet.named_modules():
        if hasattr(module, "set_lora_layer"):
            lora_layer = getattr(module, "lora_layer")
...
def hyper_compute_mergers_entropy(unet):
    entropies = []
    for name, module in unet.named_modules():
        if hasattr(module, "set_lora_layer"):
            lora_layer = getattr(module, "lora_layer")
            if lora_layer is not None:
                assert hasattr(lora_layer, "compute_mergers_similarity"), lora_layer
                entropies.append(lora_layer.compute_mergers_entropy())
    return torch.stack(entropies).sum(dim=0)
...
def initialize_merged_lora_layer_for_inference(state_dict, part, **model_kwargs):
    merged_lora_layer = MergedLoRALinearLayerInference(**model_kwargs, part=part)
    merged_lora_layer.load_state_dict(
        {
...
def insert_merged_lora_to_unet(
    unet: UNet2DConditionModel, merged_lora_name_or_path: str, **kwargs
):
    tensors = get_lora_weights(merged_lora_name_or_path, **kwargs)
    for attn_processor_name, attn_processor in unet.attn_processors.items():
        ...
        attn_module.to_q.set_lora_layer(
            initialize_merged_lora_layer_for_inference(
